...
if((isset($_POST['gps_rechercher_lieu']) || !empty($_SESSION['gps_rechercher_lieu'])) && (isset($_POST['style']) || !empty($_SESSION['communaute_rechercher_lieu'])) && (isset($_POST['adresse_rechercher_lieu']) || !empty($_SESSION['adresse_rechercher_lieu']))){
        $_SESSION['recherche']="calendrier";
        // SESSION FB
        ...
        $plateforme="";
        $proprio=0;
        $nb_jour_calendrier=7;
        // Pour les redirections autres pages
        if(isset($_SESSION['nb_jour_calendrier'])){
            $nb_jour_calendrier = $_SESSION['nb_jour_calendrier'];
            unset($_SESSION['nb_jour_calendrier']);
        }
        // Pour les changement de valeur
        if(isset($_POST['nb_jour_calendrier'])){
            $nb_jour_calendrier = intval(htmlspecialchars($_POST['nb_jour_calendrier']));
            $_SESSION['nb_jour_calendrier'] = $nb_jour_calendrier;
        ...
        if (!empty($_POST['research_radius'])){
            $research_radius = htmlspecialchars($_POST['research_radius']);
        }
        ...
        // AFFICHAGE DE LA PAGE FINALE AVEC TOUTES LES INFOS
        include_once('../ressources/vue/vueRecherche-calendrier.php');
        ...
        require('../ressources/modele/connexion_sql.php');  // Connexion Base sql
        require_once('../ressources/modele/get_extract_calendrier.php');
        $extractBdd = '';
        $extractBdd = get_extract_calendrier($latMin,$latMax,$lngMin,$lngMax,$nb_jour_calendrier); // renvoie une chaine de caract [xxx,xxxx,"xxx",xxx,...]
}
else{
        header("Refresh: 0 ; url=https://www.lepetitbal.com");
}
?>
<script data-goo="<?php echo htmlentities(json_encode($map_setup_init));?>" extractBdd="<?php echo htmlentities($extractBdd);?>" communaute="<?php echo htmlentities(json_encode($communaute));?>" recherche="<?php echo htmlentities(json_encode($recherche));?>" src="https://maps.googleapis.com/maps/api/js?callback=initMap&key=AIzaSyCVvPab8JGafCZMvlwtWEBWgUE0-3d88gs&libraries=places" async defer></script>
