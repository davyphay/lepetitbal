<html lang="fr">
    <head>
		...
		<![endif]-->		
        <title>lepetitbal, toute l'actualité locale de la danse en 3 clics</title>
    </head>
    ...
    <body class="background-cover">
		<header>
            <div id="header_left">
				...
				<div id="zone_bouton">
					<div class="dropdown">
						<button class="btn btn-primary dropdown-toggle btn-menu btn-menu-recherche" type="button" data-toggle="dropdown">Je cherche : Soirées<i class="fa fa-caret-down caret-menu-deroulant" aria-hidden="true"></i></button>
						<ul class="dropdown-menu dropdown-menu-recherche">
						  <li class="active"><a href="https://www.lepetitbal.com/soirees-et-evenements-dansants"><i class="fa fa-calendar" style="margin-right:10px"></i>Soirées ou évènements dansants</a></li>
						  <li><a href="https://www.lepetitbal.com/cours-de-danse"><i class="fa fa-graduation-cap" style="margin-right:5px"></i>Cours de danse</a></li>
					...
					<div id="zone_bouton_modifier" class="zone_bouton_actif" style="cursor:default">Modifier</div>					
				</div>
			</div>
			<div id="header_right">
			...
			<div class="modal-dialog">
				<div class="modal-content login-modal">
					<div class="modal-header login-modal-header">
						<h4 class="modal-title text-center" id="loginModalLabel">Fiche du lieu <?php echo $nom_lieu;?></h4>
					</div>
						...
						<div class="text-center">
							<div role="tabpanel" class="login-tab">
								...
								<ul class="nav nav-tabs" role="tablist">
									<li role="presentation" class="<?php if($redirection=="event_lieu"){echo "active";};?>"><a id="evenement_lieu-taba" href="#evenementLieu" aria-controls="evenementLieu" role="tab" data-toggle="tab">Évènements</a></li>
									<?php if($fiche_visible==1){;?>
										<li role="presentation" class="<?php if($redirection=="fiche_lieu"){echo "active";};?>"><a id="modifier_lieu-taba" href="#modifierLieu" aria-controls="modifierLieu" role="tab" data-toggle="tab">Fiche du Lieu</a></li>
									<?php };?>
									<li role="presentation" class="<?php if($redirection=="promotion_lieu"){echo "active";};?>"><a id="promotion_lieu-taba" href="#promouvoirLieu" aria-controls="promouvoirLieu" role="tab" data-toggle="tab">Promotion du Lieu</a></li>
									<div id="button_goback_modifier" onclick="document.location.href='<?php echo $bouton_retour?>'">Retour</div>
								</ul>
								<!-- Tab panes -->
								<div class="tab-content">
									<!-- TAB EVENEMENT LIEU -->
									<div role="tabpanel" class="tab-pane text-center <?php if($redirection=="event_lieu"){echo "active";};?>" id="evenementLieu">
										<div class="clearfix"></div>
										<div class="form-group">
											<div class="form-group encaps">
												<label>Prochains évènements programmés :</label>
												<div id="zone_event" class="zone_event">
													<?php
													if($extract[0]){
														for ($i = 0; $i < $nb_event; $i++) {
															$id_ev = $extract[$i]['id_evenement'];
															$id_lieu_ev = $extract[$i]['id_lieu_evenement'];
															//$date_event[$i]; $date_data; dans afficher lieu
															$heure_deb_ev = $extract[$i]['heure_debut_evenement'];
															$heure_fin_ev = $extract[$i]['heure_fin_evenement'];
															$titre_ev = $extract[$i]['titre_evenement'];
															$organisateur_ev = $extract[$i]['organisateur_evenement'];
															$danse_pratique_ev = $extract[$i]['danse_pratique_evenement'];
															$tarifs_ev = $extract[$i]['tarifs_evenement'];
															$description_ev = json_decode('"'.$extract[$i]['description_evenement'].'"');
															$url_fb_ev = $extract[$i]['url_fb_evenement'];
															$init_ev = $extract[$i]['initiation_evenement'];
															$special_ev = $extract[$i]['special_evenement'];
															$communaute_ev = $extract[$i]['communaute_evenement'];
															?>
															<div class="event_modifier" value="<?php echo $id_ev;?>" communaute="<?php echo $communaute_ev;?>" title="Cliquez pour modifier l'évènement">
																<div class="event_header top-border-radius">
																	<div class="event_header_left">
																		<div id="event_header_date" class="event_header_left_date" value="<?php echo $date_data[$i];?>"><?php echo $date_event[$i];?></div>
																		<div id="event_header_titre" class="event_header_left_title" value="<?php echo $titre_ev;?>"><?php echo $titre_ev;?></div>
																	</div>
																	<div class="event_header_right">
																		<div class="event_header_right_sticker">
																			<div class="event_header_right_sticker_initiation" <?php if(!$init_ev){echo 'value="0" hidden';}else{echo 'value="1"';};?>><img src="../ressources/images/website/icon-human-shoes-footprints-white.png" alt="footprints" style="width:16px;height:16px;" title="Avec initiation"></div>
																			<div class="event_header_right_sticker_special" <?php if(!$special_ev){echo 'value="0" hidden';}else{echo 'value="1"';};?>><i class="glyphicon glyphicon-star-empty" title="Évènement spécial"></i></div>
																		</div>
																		<div class="event_header_right_horaire">
																			<div id="event_header_horaire_deb" class="event_header_right_horaire_debut" value="<?php echo $heure_deb_ev;?>"><?php echo $heure_deb_ev;?></div>
																			<div class="event_header_right_separator" value="">-</div>
																			<div id="event_header_horaire_fin" class="event_header_right_horaire_fin" value="<?php echo $heure_fin_ev;?>"><?php echo $heure_fin_ev;?></div>
																		</div>
																	</div>
																</div>
																<div class="event_content">
																	<div class="event_content_footer">
																		<?php if($organisateur_ev!==""){ ;?>
																			<div id="event_content_organisateur" class="event_content_footer_info" value="<?php echo $organisateur_ev;?>"><b>Organisateur</b> <?php echo $organisateur_ev;?></div>
																		<?php } ;?>
																		<div id="event_content_danse_pratique" class="event_content_footer_info" value="<?php echo $danse_pratique_ev;?>"><b>Danses Pratiquées</b> <?php echo $danse_pratique_ev;?></div>
																		<div id="event_content_tarifs" class="event_content_footer_info" value="<?php echo $tarifs_ev;?>"><b>Tarifs</b> <?php echo $tarifs_ev;?></div>
																		<?php if ($url_fb_ev!==""){ ;?>
																			<a id="event_content_url_fb" class="event_content_footer_lien" value="<?php echo $url_fb_ev;?>" href="http://<?php echo $url_fb_ev;?>" target="_blank" title="Page Facebook de l'évènement">Lien facebook</a>
																		<?php } ;?>
																	</div>
																	<div class="event_content_description">
																		<div class="event_content_description_header"><i class="fa fa-info-circle"></i> <b>Description</b></div>
																		<div id="event_content_description_content" class="event_content_description_content" value="<?php echo $description_ev;?>"><?php echo nl2br($description_ev) ;?></div>
																	</div>
																</div>
															</div>
														<?php
														...
														echo ("<div><i>Aucun évènement programmé</i></div>");
													}
													?>
												</div>
											</div>
										</div>
										<div id="bouton_ajout_event" class="btn btn-espace-modifier-main">
											Ajouter un nouvel évènement
										</div>
										<div class="bouton" style="margin-top: 7px;"><input class="btn btn-block btn-espace-modifier btn_retour" type="button" onclick="document.location.href='<?php echo $bouton_retour?>';" value="Retour à la page de recherche"/></div>
									</div>
									<?php if($fiche_visible==1){;?>
										<!-- TAB MODIFIER LIEU -->
										<div class="margintop10"></div>
										<div role="tabpanel" class="tab-pane text-center <?php if($redirection==="fiche_lieu"){echo "active";};?>" id="modifierLieu">
											<div class="clearfix"></div>
											<form id="form_modifier_lieu" action="../ressources/controleur/action/modifierLieu.php" method="post" enctype="multipart/form-data">
												<div class="form-group">
													<label for="nom_lieu">Nom de l'organisation ou du lieu</label>
													<div class="input-group">
														<div class="input-group-addon"><i class="fa fa-home"></i></div>
														<input type="text" class="form-control" name="nom_lieu" id="nom_lieu_modif" <?php if($admin!==1)echo ('title="Veuillez contacter le support lepetitbal pour une modification" readonly');?> placeholder="Nom de l'organisation ou du lieu" maxlength="30" value="<?php echo $nom_lieu;?>" required>
														<input type="text" name="id_lieu" value="<?php echo $id_lieu;?>" hidden />
													</div>
												</div>
												<div class="form-group">
													<label for="type_lieu_modif">Type d'organisation ou de lieu</label>
													<div class="input-group">
														<div class="input-group-addon"><i class="glyphicon glyphicon-star"></i></div>
														<select name="type_lieu" class="form-control" id="type_lieu_modif" required>
															<option value="" disabled>Selectionnez le type</option>
															<option value="Ecole de Danse" <?php if($type_lieu=="Ecole de Danse")echo 'selected'?>>Ecole de Danse</option>
															<option value="Association" <?php if($type_lieu=="Association")echo 'selected'?>>Association</option>
															<option value="Salle de Sport/Club de Fitness" <?php if($type_lieu=="Salle de Sport/Club de Fitness")echo 'selected'?>>Salle de Sport/Club de Fitness</option>
															<option value="Bar/Restaurant" <?php if($type_lieu=="Bar/Restaurant")echo 'selected'?>>Bar/Restaurant</option>
															<option value="Professeur de danse" <?php if($type_lieu=="Professeur de danse")echo 'selected'?>>Professeur de danse</option>
															<option value="Autre" <?php if($type_lieu=="Autre")echo 'selected'?>>Autre</option>
														</select>
													</div>
												</div>
												<div class="form-group">
													<label for="adresse_lieu_modif">Adresse du lieu des cours/soirées/évènements</label>
													<div class="input-group">
														<div class="input-group-addon"><i class="fa fa-map-marker"></i></div>
														<input type="text" class="form-control" name="adresse_lieu" id="adresse_lieu_modif" placeholder="Adresse exacte du lieu" maxlength="60" <?php if($admin!==1)echo ('title="Veuillez contacter le support lepetitbal pour une modification" readonly');?> value="<?php echo $adresse_lieu;?>" required>
														<input type="text" name="gps_lieu" value="" hidden />
													</div>
												</div>
									
												<div class="form-group">
													<label for="communaute_lieu_modif">Styles de danse</label>
													<div class="input-group">
													  ...
													  <div class="form-control zonecheckbox" id="communaute_lieu_modif">
														<label>Afro-Latines</label>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_salsa_modif" name="communaute_lieu_salsa" value="Salsa" <?php if($communaute_lieu_salsa==1)echo 'checked'?>>Salsa Cubaine/Portoricaine</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_bachata_modif" name="communaute_lieu_bachata" value="Bachata" <?php if($communaute_lieu_bachata==1)echo 'checked'?>>Bachata</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_kizomba_modif" name="communaute_lieu_kizomba" value="Kizomba" <?php if($communaute_lieu_kizomba==1)echo 'checked'?>>Kizomba</label>
														</div>
														<label>Rock-Swing</label>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_rock4T_modif" name="communaute_lieu_rock4T" value="Rock 4T" <?php if($communaute_lieu_rock4T==1)echo 'checked'?>>Rock 4T-Moderne</label>
														</div>														
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_rock6T_modif" name="communaute_lieu_rock6T" value="Rock 6T" <?php if($communaute_lieu_rock6T==1)echo 'checked'?>>Rock 6T</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_swing_modif" name="communaute_lieu_swing" value="Swing" <?php if($communaute_lieu_swing==1)echo 'checked'?>>Swing (Lindy, Balboa, Charleston, Blues)</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_wcs_modif" name="communaute_lieu_wcs" value="WCS" <?php if($communaute_lieu_wcs==1)echo 'checked'?>>West Coast Swing</label>
														</div>
														<label>Danses de Salon</label>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_tango_modif" name="communaute_lieu_tango" value="Tango Argentin" <?php if($communaute_lieu_tango==1)echo 'checked'?>>Tango Argentin</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="communaute_lieu_salon_modif" name="communaute_lieu_salon" value="Salon" <?php if($communaute_lieu_salon==1)echo 'checked'?>>Danses de Salon</label>
														</div>														
													  </div>
													</div>
												</div>									
												...
												<div class="form-group">
													...
													<div class="input-group-addon"><img src="../ressources/images/website/icon-human-shoes-footprints.png" alt="footprints" style="width:16px;height:16px;"></div>
													  <div class="form-control zonecheckbox">
														<div class="checkbox">
															<label><input type="checkbox" id="activite_lieu_cours" name="activite_lieu_cours" value="cours" <?php if($activite_lieu_cours==1)echo 'checked'?>>Cours réguliers</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="activite_lieu_soiree" name="activite_lieu_soiree" value="soiree" <?php if($activite_lieu_soiree==1)echo 'checked'?>>Pratiques/Soirées régulières</label>
														</div>
														<div class="checkbox">
															<label><input type="checkbox" id="activite_lieu_event" name="activite_lieu_event" value="event" <?php if($activite_lieu_event==1)echo 'checked'?>>Évènements</label>
														</div>
													  </div>
													</div>
												</div>
	
												<fieldset><!--<?php //if($promotion!==1){echo ('disabled title="Vous devez promouvoir ce lieu pour pouvoir accédez à ces champs!"');};?>-->
												<div class="form-group <?php //if($promotion!==1){echo 'zone-promotion-lock';}else{echo 'zone-promotion-unlock';};?>">
													<div class="form-group">
														<label for="description_lieu">Description</label>
														<div class="input-group">
															<div class="input-group-addon"><i class="fa fa-commenting"></i></div>
															<textarea class="form-control" style="overflow:auto;resize:none" rows="5" cols="50" maxlength="500" name="description_lieu" id="description_lieu" placeholder="Décrivez le lieu (500 caractères max)"><?php echo $description_lieu;?></textarea>
														</div>
													</div>
													
													<div class="form-group">
														<label for="URL_web_lieu">Site internet ou page facebook</label>
														<div class="input-group">
															<div class="input-group-addon"><i class="fa fa-external-link"></i></div>
															<input type="text" class="form-control" name="URL_web_lieu" id="URL_web_lieu" placeholder="Exemple: lepetitbal.fr ou facebook.com/lepetitbal" value="<?php echo $URL_web_lieu;?>">
														</div>
													</div>
													
													<div class="form-group <?php //if($promotion!==1){echo 'zone-upload-photo-lock';}else{echo 'zone-upload-photo-unlock';};?>">
														<label for="image_lieu">Upload une photo</label>
														<div class="input-group" style="margin-bottom:10px;">
															<input type="hidden" name="MAX_FILE_SIZE" value="2097152"><!-- On limite le fichier à 2Mo = 2 097 152-->
															<input type="hidden" id="image_lieu_old" name="image_lieu_old" value="<?php echo $URL_photo_lieu;?>">
															<input type="file" class="filestyle" name="image_lieu" id="image_lieu" data-buttonBefore="true" data-placeholder="Format JPG/JPEG (2Mo max.)" accept="image/*" data-value="<?php echo $URL_photo_lieu;?>">
														</div>	
														<div class="input-group">
															<div id="image_preview">
																<div <?php if($URL_photo_lieu=='')echo'class="URL_photo_description_right thumbnail hidden"';else echo 'class="URL_photo_description_right thumbnail"';?>>
																	<img src="../ressources/img_upload_storage/<?php echo ($URL_photo_lieu) ?>" alt="photo storage">
																</div>
															</div>
														</div>
													</div>
												...
												<div id="bouton-planning-cours" class="planning-cours <?php if($activite_lieu_cours!==1)echo 'hidden'?>">
													<div class="caret-switch" data-toggle="collapse" href="#liste_cours_hebdo">
														<div class="zone_icone_dropdown"><i class="fa fa-caret-up fa-2x"></i></div>
														<div class="btn-xs btn-block btn-planning">Planning des cours réguliers</div>
													</div>
													<div id="liste_cours_hebdo" class="collapse in">
														<div class="form-group-collapse">
														 <?php echo $extractCoursHebdoTableForm;?>								 
													   </div>
														<input type="text" name="id_cours_hebdo" value="<?php echo $id_cours_hebdo?>" hidden />
														<input type="hidden" id="cours_changement" name="cours_changement" value="" />
													</div>
												</div>
												
												<div id="bouton-planning-soiree" class="planning-cours <?php if($activite_lieu_soiree!==1)echo 'hidden'?>">
													<div class="caret-switch" data-toggle="collapse" href="#liste_soiree_hebdo">
														<div class="zone_icone_dropdown"><i class="fa fa-caret-up fa-2x"></i></div>
														<div class="btn-xs btn-block btn-planning">Planning des pratiques et soirées régulières</div>
													</div>
													<div id="liste_soiree_hebdo" class="collapse in">
														<div class="form-group-collapse">
														 <?php echo $extractSoireeHebdoTableForm;?>								 
													   </div>
														<input type="text" name="id_soiree_hebdo" value="<?php echo $id_soiree_hebdo?>" hidden />
														<input type="hidden" id="soiree_changement" name="soiree_changement" value="" />													
													</div>
												</div>
												<div class="separator-5px"></div>
												<div class="form-group">
													<div class="bouton"><input id="btn_modifier_lieu" class="btn btn-block btn-espace-modifier-main" type="button" value="Modifier la fiche du lieu"/></div>
													<div class="bouton"><input id="btn_redirection_promotion_lieu" class="btn btn-block btn-espace-modifier" type="button" value="Promotion du lieu"/></div>
													<div class="bouton"><input class="btn btn-block btn-espace-modifier btn_retour" type="button" onclick="document.location.href='<?php echo $bouton_retour?>';" value="Retour à la page de recherche"/></div>												
												</div>
											</form>
										</div>
									<?php };?>
									<!-- TAB promotion lieu -->
									<div class="margintop10"></div>
									<div role="tabpanel" class="tab-pane text-center <?php if($redirection=="promotion_lieu"){echo "active";};?>" id="promouvoirLieu">
										<div class="clearfix"></div>
										<form id="form_promouvoir_lieu" action="../ressources/controleur/action/promouvoir_lieu.php" method="post">
											<div class="form-group">
												<label for="nom_lieu">Nom de l'organisation ou du lieu</label>	
												<div class="input-group">
													<div class="input-group-addon"><i class="fa fa-home"></i></div>
													<input type="text" class="form-control" name="nom_lieu" id="nom_lieu_prom" disabled value="<?php echo $nom_lieu;?>" required>
													<input type="text" name="id_lieu" value="<?php echo $id_lieu;?>" hidden />
													<input type="text" name="boolean_proprio" value="<?php echo $boolean_proprio;?>" hidden />
												</div>
											</div>
											<div class="form-group">
											<label for="statut_lieu">Statut</label>												
												<div class="input-group">
													<div class="input-group-addon"><i class="glyphicon glyphicon-bell"></i></div>
													<input type="text" id="statut_lieu" class="form-control" value="<?php echo $statut_lieu ;?>" disabled>
												</div>
											</div>
											<div class="form-group">
											<label for="nb_jetons_dispo_prom">Jetons disponibles</label>				
												<div class="input-group">
													...
													<input type="text" id="nb_jetons_dispo_prom" class="form-control" value="<?php echo $nb_jeton_dispo ;?>" disabled>
													<div id="bouton_obtenir_jeton_prom" class="input-group-addon bouton_obtenir_jeton" style="font-size:12px;cursor:pointer">Obtenir des jetons</div>
												</div>
											</div>
											<div class="form-group">
											<label for="nb_jetons_utilise">Nombre de jours de promotion / Dépense en jetons</label>
												<div class="input-group">
													...
													<input type="number" id="nb_jetons_utilise" name="nb_jetons_utilise" class="form-control" max="<?php echo $nb_jeton_dispo ;?>" value="0" placeholder="Sélectionnez un nombre de jetons">
													<span class="input-group-addon btn-number inc"><span class="glyphicon glyphicon-chevron-up"></span></span>
													<span class="input-group-addon btn-number dec"><span class="glyphicon glyphicon-chevron-down"></span></span>
												</div>
											</div>
											<div class="form-group">
												<label for="date_fin_promotion">Estimation date de fin de promotion</label>
												<input type="text" id="date_fin_promotion_bdd" name="date_fin_promotion_bdd" value="<?php echo $date_expiration_promotion_lieu;?>" hidden />
												<div class="input-group">
													<div class="input-group-addon"><i class="fa fa-calendar" aria-hidden="true"></i></div>
													<input type="text" id="date_picker" name="date_picker" class="form-control" placeholder="Date au format JJ-MM-AAAA">
												</div>
											</div>
											<div class="form-group">
												<div class="bouton"><input id="btn_promouvoir_lieu" class="btn btn-block btn-espace-modifier-main" type="button" value="Promouvoir ce lieu" /></div>
												<div class="bouton"><input id="btn_redirection_fiche_lieu" class="btn btn-block btn-espace-modifier" type="button" value="Retour à la fiche du lieu" /></div>
												<div class="bouton"><input class="btn btn-block btn-espace-modifier btn_retour" type="button" onclick="document.location.href='<?php echo $bouton_retour?>';" value="Retour à la page de recherche" /></div>
											</div>
										</form>
									</div>
											...
											<div role="tabpanel" class="tab-pane text-center" id="ajouter_lieu_devenir_annonceur">
												...
												<form id="form_ajout_lieu_demande" action="../ressources/controleur/action/devenir_annonceur.php" method="post" enctype="multipart/form-data">
													...
													<div class="scroll-overflow-reg">
														...
														<div class="form-group">
															<label for="activite_lieu">Activités</label>
															<div class="input-group">
															<div class="input-group-addon"><img src="/ressources/images/website/icon-human-shoes-footprints.png" alt="footprints" style="width:16px;height:16px;"></div>
															<div class="form-control zonecheckbox">
																  <div class="checkbox">
												...
												<div class="form-group zone-event-unlock">
													...
													<div class="form-group">
														...
														<div class="input-group">
															...
															<input type="hidden" id="cout_jeton" value="0">
		...
		<!-- Espace modifier event MODAL -->
        <div class="modal fade" id="espaceEventModal" tabindex="-1" role="dialog" aria-labelledby="eventModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content login-modal">
                    <div class="modal-header login-modal-header_event">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title text-center" id="eventModalLabel">Modifier évènement</h4>
                    </div>
                    <div class="modal-body">
                        <div class="text-center">
                            <div class="tab-pane">
                                <form id="form_modifier_evenement" action="../ressources/controleur/action/modifier_evenement.php" method="post">
									<input type="hidden" id="id_evenement" name="id_evenement" value="" />
									<input type="text" name="id_lieu_evenement" value="<?php echo $id_lieu;?>" hidden />
									<div class="form-group zone-event-unlock">
										<div class="form-group">
											<label>Date - Horaire début/fin</label>
											<div class="input-group event-timeline">
												<div class="input-group-addon"><i class="fa fa-calendar"></i></div>
												<input type="text" id="date_evenement" name="date_evenement" class="form-control" placeholder="Date au format JJ-MM-AAAA">
												<div class="input-group-addon"><i class="glyphicon glyphicon-time"></i></div>
												<input type="text" class="form-control timepicker" name="heure_debut_evenement" id="heure_debut_evenement" required>
												<div class="input-group-addon"><i class="glyphicon glyphicon-time"></i></div>
												<input type="text" class="form-control timepicker" name="heure_fin_evenement" id="heure_fin_evenement">
											</div>
										</div>
										<div class="form-group">	
											<label>Titre</label>
											<div class="input-group">
												<input type="text" class="form-control" name="titre_evenement" id="titre_evenement" placeholder="Ajoutez un titre" maxlength="50" required>
											</div>
										</div>
										<div class="form-group">	
											<label>Organisateur</label>
											<div class="input-group">
												<div class="input-group-addon"><i class="fa fa-bullhorn"></i></div>
												<input type="text" class="form-control" name="organisateur_evenement" id="organisateur_evenement" placeholder="Indiquez l'organisateur (si différent du propriétaire du lieu)" maxlength="50">
											</div>
										</div>										
										<div class="form-group">	
											<label>Danses pratiquées</label>
											<div class="input-group">
												<div class="input-group-addon"><img src="../ressources/images/website/icon-danse-transparent.png" alt="icon danse" style="width:16px;height:16px;"></div>
												<input type="text" class="form-control" name="danse_pratique_evenement" id="danse_pratique_evenement" placeholder="Indiquez les danses pratiquées" maxlength="50" required>
											</div>
										</div>
										<div class="form-group">	
											<label>Tarifs</label>
											<div class="input-group">
												<div class="input-group-addon"><i class="fa fa fa-eur"></i></div>
												<input type="text" class="form-control" name="tarifs_evenement" id="tarifs_evenement" placeholder="Indiquez les tarifs de l'évènement" maxlength="50" required>
											</div>
										</div>
										<div class="form-group">	
											<label>Description</label>
											<div class="input-group">
												<div class="input-group-addon"><i class="glyphicon glyphicon-edit"></i></div>
												<textarea class="form-control" style="overflow:auto;resize:none" rows="5" maxlength="2000" name="description_evenement" id="description_evenement" placeholder="Ajoutez une description: Infos, Contact, Prix... (2000 caractères max)"></textarea>
											</div>
										</div>
										<div class="form-group">
											<label for="url_fb_evenement">Lien facebook</label>
											<div class="input-group">
												<div class="input-group-addon"><i class="fa fa-facebook"></i></div>
												<input type="text" class="form-control" name="url_fb_evenement" id="url_fb_evenement" placeholder="Exemple: facebook.com/events/XXXX/">
											</div>
										</div>
										<div class="form-group">
											<label for="option_evenement">Options</label>
											<div class="form-control zonecheckbox">
												<div class="checkbox">
													<label>
														<input type="checkbox" id="initiation_evenement" name="initiation_evenement" value="initiation">
														<img src="../ressources/images/website/icon-human-shoes-footprints-white.png" alt="footprints white" style="width:16px;height:16px;margin-right:5px;">Initiation/Stage
													</label>
												</div>
												<div class="checkbox">
													<label>
														<input type="checkbox" id="special_evenement" name="special_evenement" value="special">
														<i class="glyphicon glyphicon-star-empty" style="margin-right:8px;font-size:14px; margin-top:1px;"></i>Évènement Spécial (concert, démonstration, show, ...)
													</label>
												</div>
											</div>
										</div>
										<div class="form-group">
											<label for="communaute_evenement">Styles de danse</label>
											<div class="input-group">
												<div class="input-group-addon"><img src="../ressources/images/website/icon-danse-transparent.png" alt="icon danse" style="width:16px;height:16px;"></div>
												<div class="form-control zonecheckbox" id="communaute_evenement">
													<label>Afro-Latines</label>
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_salsa" name="communaute_evenement_salsa" value="Salsa">Salsa Cubaine-Portoricaine</label>
													</div>
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_bachata" name="communaute_evenement_bachata" value="Bachata">Bachata</label>
													</div>
													<div class="checkbox">
													 <label><input type="checkbox" id="communaute_evenement_kizomba" name="communaute_evenement_kizomba" value="Kizomba">Kizomba</label>
													</div>
													<label>Rock-Swing</label>
													 <div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_rock4T" name="communaute_evenement_rock4T" value="Rock 4T">Rock 4T-Moderne</label>
													</div>															 
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_rock6T" name="communaute_evenement_rock6T" value="Rock 6T">Rock 6T</label>
													</div>
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_swing" name="communaute_evenement_swing" value="Swing">Swing-Lindy-Balboa-Charleston-Blues</label>
													</div>
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_wcs" name="communaute_evenement_wcs" value="WCS">West Coast Swing</label>
													</div>
													<label>Danses de salon</label>
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_tango" name="communaute_evenement_tango" value="Tango Argentin">Tango Argentin</label>
													</div>
													<div class="checkbox">
													  <label><input type="checkbox" id="communaute_evenement_salon" name="communaute_evenement_salon" value="Salon">Danses de salon</label>
													</div>
												</div>
											</div>
										</div>
									</div>
                                    <div class="margintop10 bouton"><input id="btn_modifier_evenement" class="btn btn-block btn-espace-modifier-event" type="button" value="Modifier l'évènement" /></div>
									<div class="margintop10 bouton"><input id="btn_supprimer_evenement" class="btn btn-block btn-espace-modifier-event" type="button" value="Supprimer l'évènement" /></div>
									<div class="margintop10 bouton"><input id="btn_retour_evenement" class="btn btn-block btn-espace-modifier" type="button" value="Retour" /></div>
								</form>
								<form id="form_supprimer_event" action="../ressources/controleur/action/supprimer_evenement.php" method="post" >
									<input type="hidden" id="id_evenement_delete" name="id_evenement_delete" value="" />
									<input type="text" name="id_lieu_evenement" value="<?php echo $id_lieu;?>" hidden />
								</form>
                            </div>
                        </div>
		...
		<script src="../ressources/controleur/js/boutonentrer.js"></script> 
		<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCVvPab8JGafCZMvlwtWEBWgUE0-3d88gs&signed_in=true" async defer></script>
		...
		<script src="../ressources/controleur/js/addinputs.js"></script>
		<script src="../ressources/controleur/js/caret-switch.js"></script>
		<script src="../ressources/controleur/js/menu.js"></script>
		<script src="../ressources/controleur/js/promotion_lieu.js"></script>
		...
		<script src="../ressources/controleur/js/event_modifier.js"></script>
    </body>
</html>
